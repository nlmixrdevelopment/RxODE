rxodeTest({
  ...
  test_that("issue #229", {
    ...
    model <- RxODE({
      ...
      rx_pred_ = rx_expr_5
      rx_r_ = 1e+06 * Rx_pow_di((rx_expr_1 * Cent_AKR), 2) * Rx_pow_di(THETA[5],
                                                                       2) + Rx_pow_di(THETA[6], 2)
      lKa = THETA[1]
      lCL = THETA[2]
      lVc = THETA[3]
      F_SC = THETA[4]
    ...
    parm <- setNames(c(-3.54456675092961, -2.30258509299405, 1.09861228866811, 0.3, 0.2, 10),
                     paste0("THETA[", 1:6, "]"))
