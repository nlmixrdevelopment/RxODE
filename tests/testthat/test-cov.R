...
rxPermissive({
    ...
    for (m in c("liblsoda", "lsoda", "dop853")){
        context(sprintf("Simple test for time-varying covariates (%s)", method=m))
        ...
        out <- rxSolve(ode,
                       ...
                       events = et,
                       inits = c(X=1, Y=1, Z=1),
                       covs = cov, add.cov=TRUE, method=m);
        sink();
        ...
        out <- rxSolve(ode,
                       ...
                       inits = c(X=1, Y=1, Z=1),
                       covs = cov,
                       covs_interpolation="NOCB", add.cov=TRUE, method=m);
        sink()
        lin.interp <- read.csv("temp.csv");
        unlink("temp.csv");
        ...
        out <- rxSolve(ode,
                       ...
                       inits = c(X=1, Y=1, Z=1),
                       covs = cov,
                       covs_interpolation="midpoint", add.cov=TRUE, method=m);
        sink()
        lin.interp <- read.csv("temp.csv");
        unlink("temp.csv");
        ...
        out <- rxSolve(ode,
                       ...
                       inits = c(X=1, Y=1, Z=1),
                       covs = cov,
                       covs_interpolation="constant", add.cov=TRUE, method=m);
        sink()
        lin.interp <- read.csv("temp.csv");
        unlink("temp.csv");
        ...
        out1 <-
            rxSolve(ode,
                    ...
                    events = et,
                    inits = c(X=1, Y=1, Z=1), add.cov=TRUE, method=m)
        sink();
        unlink("temp");
        ...
        context(sprintf("Test First Assignment (%s)", method=m))
        ...
        context(sprintf("Test Second Assignment (%s)", method=m))
        sink("temp")
        out$a <- out3$a
        sink();
        unlink("temp");
