rxPermissive({
    ...
    test_that("No infusions are in the ODE.", {
        expect_equal(structure("__DDtStateVar__[0] = _InfusionRate(0, _solveData, _cSub) + a*X+Y*Z;\n__DDtStateVar__[1] = _InfusionRate(1, _solveData, _cSub) + b*(Y-Z);\n__DDtStateVar__[2] = _InfusionRate(2, _solveData, _cSub) + -X*Y+c*Y-Z;\n", .Names = "parseModel"),
    ...
    test_that("No infusions are in the ODE.", {
        expect_equal(structure("__DDtStateVar__[0] = _InfusionRate(0, _solveData, _cSub) + a*X+Y*Z+0.0;\n__DDtStateVar__[1] = _InfusionRate(1, _solveData, _cSub) + b*(Y-Z)+0.0;\n__DDtStateVar__[2] = _InfusionRate(2, _solveData, _cSub) + -X*Y+c*Y-Z+0.0;\n", .Names = "parseModel"),
    ...
    test_that("Can reference prior infusions in current d/dt()", {
        expect_equal(structure("__DDtStateVar__[0] = _InfusionRate(0, _solveData, _cSub) + a*X+Y*Z+0.0;\n__DDtStateVar__[1] = _InfusionRate(1, _solveData, _cSub) + b*(Y-Z)+_InfusionRate(0, _solveData, _cSub);\n__DDtStateVar__[2] = _InfusionRate(2, _solveData, _cSub) + -X*Y+c*Y-Z+_InfusionRate(0, _solveData, _cSub);\n", .Names = "parseModel"), rxModelVars(rx3)$model["parseModel"])
    })
