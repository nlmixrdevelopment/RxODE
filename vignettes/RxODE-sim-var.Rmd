...
```{r}
if (file.exists("sim-var-sim.rds")){
    load("sim-var-sim.rds")
} else {
    sim  <- rxSolve(mod,theta,ev,omega=omega,nSub=100)
    save(sim, file = "sim-var-sim.rds")
}
```
...
if (file.exists("sim-var-s2.rds")){
    ...
    save(sim, s, file = "sim-var-s2.rds");
}
plot(s)
```
...
if (file.exists("sim-var-s3.rds")){
    ...
    s <-sim %>% confint(c("centr", "eff"))
    save(sim, s, file = "sim-var-s3.rds");
...
if (file.exists("sim-var-s4.rds")){
    ...
    save(sim, s, file = "sim-var-s4.rds")
}
