...
are more familiar with NONMEM events you could use them directly with
the event table function `et`
...
For more information about creating complex dosing regimens, you can
see the RxODE events vignette.
...
```
Note that the labels are automatically labeled with the units from the initial event table. This is because 
RxODE uses the `units` package to handle units and `ggforce` to handle the plotting.
