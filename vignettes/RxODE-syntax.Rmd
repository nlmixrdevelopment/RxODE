...
Statements can be either assignments, conditional `if` or `while`
statements, or printing statements (for debugging/testing). Assignment
statements can be:
...
A brief description of the built-in functions is
in the following table:

```{r}
clLinCmt <- read.csv("syntax-functions.csv");
library(DT)
datatable(clLinCmt, rownames = FALSE, filter="top", options = list(pageLength = 10, scrollX=T) )
```
