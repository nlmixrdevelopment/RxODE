...
This is similar to modeled duration.  This is equaivlent to NONMEM's
...
(i.e. morning 100 mg vs evening 150 mg). This is done by:
 ...
 - Adding back all the prior stat values
...
To turn back on the compartment, a zero-dose to the compartment would be needed.

```{r}
ev <- et(timeUnits="hr") %>%
    et(amt=10000, ii=12, addl=3) %>%
    et(time=6, cmt="-eff") %>%
    et(time=12,amt=0, cmt="eff") %>%
    et(seq(0, 24, length.out=100))
