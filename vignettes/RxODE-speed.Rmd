...
bench <- eval(parse(text=sprintf("microbenchmark(%s)",
                                     paste(paste0("runThread(", seq(1, rxCores() * 2),")"),
                                           collapse=","))))
print(bench)
```
...
the data-set for `nlmixr` comparisons:
...
things to keep in mind:
 ...
 - `RxODE` uses `mvnfast` for simulation of `eta` values.  This uses
   the thread safe sumo random number generator.  Therefore the
   results are expected to be different (also the random samples are
   taken in a different order which would be different)
