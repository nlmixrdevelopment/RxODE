c 04MAR2014 wwang 
c modified by wwang due to identical & duplicated subroutines as in subbim.f: 
c 	DEC, DECB, SOL, SOLB
c these subroutines are disabled in this file.
c-----------------------------------------------------------------------
...
C
      SUBROUTINE DECDSABL (N, NDIM, A, IP, IER)
...
C
C
      SUBROUTINE SOLDSABL (N, NDIM, A, B, IP)
C VERSION REAL DOUBLE PRECISION
...
C----------------------- END OF SUBROUTINE SOLHC -----------------------
      END
C
      SUBROUTINE DECBDSBL (N, NDIM, A, ML, MU, IP, IER)
      REAL*8 A,T
      DIMENSION A(NDIM,N), IP(N)
C-----------------------------------------------------------------------
...
C
C
      SUBROUTINE SOLBDSBL (N, NDIM, A, ML, MU, B, IP)
      REAL*8 A,B,T
      DIMENSION A(NDIM,N), B(N), IP(N)
C-----------------------------------------------------------------------
C  SOLUTION OF LINEAR SYSTEM, A*X = B .
