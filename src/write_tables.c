...
save_binary_tables(File *file) {
  ...
  if (file->str) {
    file->cur_str = *file->str = (unsigned char*) R_chk_calloc(len,sizeof(unsigned char*));
    *file->str_len = len;
  }
...
new_offset(File *fp, char *name) {
  OffsetEntry *entry = Calloc(1,OffsetEntry);
...
write_scanner_data(File *fp, Grammar *g, char *tag) {
  ...
  vsblock = Calloc((nvsblocks ? nvsblocks : 1), ScannerBlock);
  for (i = 0; i < 4; i++) {
...
write_goto_data(File *fp, Grammar *g, char *tag) {
  ...
  goto_valid = R_chk_calloc(nvalid_bytes,sizeof(int));
  vec_clear(&vgoto);
  for (i = 0; i < g->states.n; i++) {
    s = g->states.v[i];
    if (s->gotos.n) {
...
write_header(Grammar *g, char *base_pathname, char *tag) {
  ...
  if (g->write_header > 0 || (g->write_header < 0 && (tokens || states))) {
    ...
    fprintf(hfp,"  RxODE available at https://github.com/hallowkm/RxODE\n");
			     ...
			     FILE *fp, unsigned char **str, unsigned int *str_len)
{
  ...
  if (!binary) {
    ...
    d_version(ver);
    fprintf(fp, "/*\n  Generated by RxODE\'s mkdparse a port of Make DParser Version %s\n", ver);
    fprintf(fp,"  RxODE available at https://github.com/hallowkm/RxODE\n");
