...
List etEmpty(CharacterVector units){
  ...
  e["nobs"] = 0;
  e["ndose"] = 0;
  e[".show"] = LogicalVector::create(_["id"] = false, _["time"] = true,
				     _["low"] = false, _["high"] = false,
...
// List etAddDose(double time, std::string cmt,  double amt, double rate, double ii, int addl, int evid, int ss){
// }
...
List etAddTimes(NumericVector newTimes, int idMax, std::string cmt, bool turnOnShowCmt, List curEt){
  ...
  for (i = idx.size(); i--;){
    ...
    tmpN = as<NumericVector>(lst[1]); // time
    tmpN[i] = time[idx[i]];
    if (idx[i] >= oldSize){
      // low
      tmpN = as<NumericVector>(lst[2]);
      tmpN[i] = NA_REAL;
  ...
  cls.attr(".RxODE.lst") = e;
  lst.attr("class") = cls;
  lst.attr("row.names") = IntegerVector::create(NA_INTEGER, -as<int>(e["nobs"]));
  return lst;
}
...
List etResizeId(int maxId, List curEt){
  ...
  newEt.attr("row.names") = IntegerVector::create(NA_INTEGER, -as<int>(e["nobs"]));
  return newEt;
}
