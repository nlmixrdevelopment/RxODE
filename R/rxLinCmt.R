    ...
    if (length(.w) == 0){
        ...
        if (.derived){
          return(sprintf(paste0(".Call(`_calcDerived`,as.integer(%d),as.integer(%d),with(.lst,{", paste(.lines, collapse=";\n"),
                        ";\nif (verbose){message(\"Detected %d-compartment model\")};list(as.double(rx_p1), as.double(rx_v1), as.double(rx_p2), as.double(rx_p3), as.double(rx_p4), as.double(rx_p5), as.double(%s), as.double(rx_tlag), as.double(rx_tlag2), as.double(rx_F), as.double(rx_F2), as.double(rx_rate), as.double(rx_dur), as.double(rx_rate2), as.double(rx_dur2))}),digits)"), .trans, .ncmt, .ncmt, ifelse(.oral, "rx_ka", "0.0")))
        }
        if (linCmtSens){
